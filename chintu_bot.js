...
app.get('/', (req, res) => res.send('🤖 Chintu Bot Online — Scan QR from logs'));
...
const CONFIG = {
    prefix: '.',
    adminNumber: '919412185706', // YOUR NUMBER with country code
    botName: 'Chintu Bot',
    emojiReactions: true,
};
...
async function connectToWhatsApp() {
    ...
    try {
        ...
        sock = makeWASocket({
            ...
            printQRInTerminal: true, // THIS GENERATES QR IN LOGS
        });
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (qr && !qrDisplayed) {
                ...
                console.log('║                                                            ║');
                console.log('║   Open WhatsApp → Link Devices → Scan QR                   ║');
                console.log('║                                                            ║');
                console.log('╚════════════════════════════════════════════════════════════╝\n');
                console.log('🔗 QR CODE:');
                console.log(qr);
                console.log('\n');
                console.log('💡 If QR doesn\'t display properly, copy the QR string above and use:');
            ...
            if (connection === 'close') {
                ...
                isConnecting = false;
                setTimeout(sendStartupMessage, 2000);
            }
        });
...
console.log('📱 QR Code will appear in logs...');
console.log('📱 Open WhatsApp → Link Devices → Scan QR\n');
connectToWhatsApp();
